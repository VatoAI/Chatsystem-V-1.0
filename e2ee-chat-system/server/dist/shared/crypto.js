...
class CryptoManager {
     ...
     */
    static async deriveSharedSecret(privateKeyBase64, publicKeyBase64) {
        try {
            const privateKeyBuffer = this.base64ToBuffer(privateKeyBase64);
            const publicKeyBuffer = this.base64ToBuffer(publicKeyBase64);
            const privateKey = await crypto.subtle.importKey('pkcs8', privateKeyBuffer, { name: 'ECDH', namedCurve: 'P-256' }, false, ['deriveKey', 'deriveBits']);
            const publicKey = await crypto.subtle.importKey('spki', publicKeyBuffer, { name: 'ECDH', namedCurve: 'P-256' }, false, []);
     ...
     */
    static async encryptMessage(message, privateKeyBase64, publicKeyBase64) {
        try {
            const sharedSecret = await this.deriveSharedSecret(privateKeyBase64, publicKeyBase64);
            const encoder = new TextEncoder();
            const data = encoder.encode(message);
            const iv = crypto.getRandomValues(new Uint8Array(12)); // 96-bit IV for GCM
            const encryptedData = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, sharedSecret, data);
     ...
     */
    static async decryptMessage(encryptedMessageBase64, privateKeyBase64, publicKeyBase64) {
        try {
            const sharedSecret = await this.deriveSharedSecret(privateKeyBase64, publicKeyBase64);
            ...
            const decryptedData = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, sharedSecret, encryptedData);
